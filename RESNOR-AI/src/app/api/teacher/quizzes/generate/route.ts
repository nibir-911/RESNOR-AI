...
export async function POST(request: Request) {
  try {
    ...
    // Preview mode: return questions without saving to DB
    if (preview) {
      return NextResponse.json({
        ...
        message: usedFallback
          ...
          : `Generated ${mappedQuestions.length} questions for "${topic}". Review and click Save to publish.`,
      })
    }
    ...
    return NextResponse.json({
      ...
      message: usedFallback
        ? 'AI generation unavailable. Used template questions instead.'
        : `AI-generated quiz "${quiz.title}" created with ${mappedQuestions.length} questions.`,
    })
  } catch (error) {
